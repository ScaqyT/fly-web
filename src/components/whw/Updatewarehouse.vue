<template>
   <div>
      <el-form :model="form" ref="form" :rules="rules" label-width="80px" :inline="false" size="normal">
         ...
         <el-form-item>
            <el-button type="primary" @click="Updatewarehouse('form')">确定</el-button>
...
export default {
   ...
   mounted(){
    ...
    methods:{
          ...
          this.$refs[formName].validate(valid=>{
             if(valid){
                this.$confirm('是否确定更新', {
                   ...
                   this.putRequest("/warehouse/w/info/",this.form).then(resp=>{
                      if(resp.code==200){
                         this.$message({
                         message: resp.msg,
                         type: 'success'
       ...
       initwarehouse(){
          this.getRequest('/warehouse/w/info/?id='+this.updateId).then(resp=>{
             if(resp){
                this.form = resp[0];
             }
          })
